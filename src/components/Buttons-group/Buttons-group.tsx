...
import modalStyles from "../Films-list/Films-list.module.css";
import { FunctionComponent, useState } from "react";
import { Modal } from "../Modal/Modal";

export const ButtonGroup: FunctionComponent<MovieCutI> = (data) => {
  const dispatch = useDispatch();
  const productAmount = useSelector(state => selectProductAmount(state, data.id));
  ...
  return (
    <div className={styles.button_group}>
      <button onClick={productAmount === 1 ? () => handleOpenModal() : () => dispatch(cartActions.decrement(data.id))} 
      className={productAmount ? styles.orange_button : styles.orange_blocked_button}>
      ...
      <button onClick={() => dispatch(cartActions.increment(data))} 
      className={productAmount !== 30 ? styles.orange_button : styles.orange_blocked_button}>
        <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {visible && (
        <Modal header="Удаление билета" onClose={handleCloseModal}>
          <div className={modalStyles.modal_content_container}>
            <span className={modalStyles.modal_text}>
              Вы уверены, что хотите удалить билет?
            </span>

            <div className={modalStyles.button_group}>
              <button className={modalStyles.button_orange} onClick={() => {dispatch(cartActions.remove(data.id)), handleCloseModal()}}>
                <span className={modalStyles.button_orange_text}>
                  Да
                </span>
              </button>

              <button className={modalStyles.button_white}>
                <span className={modalStyles.button_white_text} onClick={() => handleCloseModal()}>
                  Нет
                </span>
              </button>
            </div>
