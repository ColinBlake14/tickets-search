  ...
  return (
    <div className={styles.comment_container}>
      ...
      <div className={styles.info_container}>
        ...
        <span className={styles.bot_info_container}>
          {text}
        </span>
      </div>
    </div>
...
export const Comments: FunctionComponent<{ movieId: string }> = ({ movieId }) => {
  const {data, isLoading, error} = useGetCommentsByIdQuery(movieId);
  ...
  return (
    <div className={styles.comments_container}>
      { data.map((comment, index) => <Comment {...comment} key={index}/>) }
    </div>
  )
}
